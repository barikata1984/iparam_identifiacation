...
## 2026-06-08: FT 300-S + teleop モードでの初回実行

### バグ修正

`phase_identify()` L726 で `gripper_cal` (未定義ローカル変数) を参照して NameError.
`self._gripper_cal_params` に修正.

### 推定結果 (FT 300-S, teleop, 物体把持, 322 フレーム)

| param | OLS | TLS | OLS+bias | TLS+bias |
|---|---|---|---|---|
| m | 0.613 | 0.878 | 1.275 | 1.328 |

差分法による Object 推定:

| param | OLS | TLS | OLS+bias | TLS+bias |
|---|---|---|---|---|
| m | -0.397 | -0.132 | 0.265 | 0.318 |

- OLS/TLS (バイアスなし): 負の質量. センサオフセットを吸収できず破綻.
- OLS+bias/TLS+bias: 物体質量 0.27–0.32 kg.
- バイアス Fz が -9.2 ~ -9.5 N と大きい.

### 理論的考察: 零点補正とバイアス推定の関係

零点補正は動力学モデルに定数オフセット `-V_static(g₀)·φ` を導入する.
この項は定数 6 次元ベクトルなので、Approach 2 のバイアス項 `b` が正確に吸収する:

```
W_zeroed(t) = V(t)·φ - V_static(g₀)·φ = V(t)·φ + b,  where b = -V_static(g₀)·φ
```

V_ginit (Approach 1) はリグレッサ側で同じ定数を補正する (`V_offs = V + V_ginit`).
両者は同じ問題を入口と出口の異なる場所で解いている.

対比: FT 300-S ファームウェアの姿勢追従型重力補償は `V_gravity(g(t))·φ_tool` と
毎時刻変化するため、定数バイアスでは吸収不可能 (I-12 の根本原因).
