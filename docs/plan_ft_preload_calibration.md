...
`ft_raw_wrench` (RTDE) はゼロ化されないが、構造的プリロード (~25000、未較正の生単位であり N ではない) が含まれる。
注: `ft_raw_wrench` は較正前の生カウントなので SI (N/Nm) の同定には直接使えない。詳細は `LOGS/log_ft_sensor_behavior.md` 参照。
