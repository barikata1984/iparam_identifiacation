...
class Tool0KinematicsCalculator:
    ...
    def compute_with_given_acceleration(
        self, q: np.ndarray, v: np.ndarray, a: np.ndarray
    ) -> dict:
        """
        Compute tool0 classical velocities and accelerations with given joint acceleration.

        This method does not use numerical differentiation and is useful for
        verification with analytically known accelerations.
        ...
        a : np.ndarray
            Joint accelerations [rad/s²], shape (6,)
        ...
        return {
            "angular_velocity": twist.angular.copy(),
            "linear_velocity": twist.linear.copy(),
            "angular_acceleration": classical_acc.angular.copy(),
            "linear_acceleration": classical_acc.linear.copy(),
            "joint_position": q.copy(),
            "joint_velocity": v.copy(),
            "joint_acceleration": a.copy(),
        }

    def verify_classical_acceleration(
        self, q: np.ndarray, v: np.ndarray, a: np.ndarray, atol: float = 1e-10
    ) -> bool:
        """
        Verify that classical acceleration equals spatial acceleration + ω × v.
        ...
        spatial_acc = pin.getFrameAcceleration(
            self.model, self.data, self.tool0_id, pin.ReferenceFrame.LOCAL
        )
        classical_acc = pin.getFrameClassicalAcceleration(
            self.model, self.data, self.tool0_id, pin.ReferenceFrame.LOCAL
        ...
        # classical_linear = spatial_linear + ω × v
        computed_classical_linear = spatial_acc.linear + np.cross(
            twist.angular, twist.linear
        )

        return np.allclose(classical_acc.linear, computed_classical_linear, atol=atol)
